...
function bowlingScore(frames) {
    ...
    for (let i = 0; i < newArr.length; i++) {
      if (newArr[i] === "X") {
        scoreArr.push(...[10, newArr[i + 1] === undefined ? 0 : convert(newArr[i + 1], newArr, i + 1), newArr[i + 2] === undefined ? 0 : convert(newArr[i + 2], newArr, i + 2)])
      } else if (newArr[i] === "/") {
          scoreArr.push(...[10 - newArr[i - 1], newArr[i + 1] === undefined ? 0 : convert(newArr[i + 1], newArr, i + 1)])
      } else {
          scoreArr.push(parseInt(newArr[i]))
      }
    }
    console.log(scoreArr)
    return normalize(scoreArr.reduce(function(a, b){
          return a + b
      }, 0))
  }